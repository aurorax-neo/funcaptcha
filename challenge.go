...
func (sn *Session) RequestChallenge(isAudioGame bool) (*ApiBreaker, error) {
	challenge_request := requestChallenge{
		Sid:               sn.Sid,
		Token:             sn.SessionToken,
		AnalyticsTier:     40,
		RenderType:        "canvas",
		Lang:              "en-us",
	...
	err = sn.log(challenge_data.ChallengeID, challenge_data.GameData.GameType, "loaded", "game loaded")
	sn.Challenge = challenge_data
	...
	sn.ConciseChallenge = ConciseChallenge{
...
func (sn *Session) SubmitAnswer(indices []int, isAudio bool, apiBreaker *ApiBreaker) error {
	submission := submitChallenge{
		SessionToken:  sn.SessionToken,
		Sid:           sn.Sid,
		GameToken:     sn.Challenge.ChallengeID,
		RenderType:    "canvas",
		AnalyticsTier: 40,
	...
	submission.Guess = Encrypt(answer, sn.SessionToken)
	...
	req.Header = sn.Headers
	req.Header.Set("X-Requested-ID", getRequestId(sn.SessionToken))
	req.Header.Set("X-NewRelic-Timestamp", getTimeStamp())
	...
	// Set new client
	cli, _ := tls_client.NewHttpClient(tls_client.NewNoopLogger(), sn.Options...)
	sn.Client = &cli
	return nil
}

func (sn *Session) log(game_token string, game_type int, category, action string) error {
	v := sn.ChallengeLogger
